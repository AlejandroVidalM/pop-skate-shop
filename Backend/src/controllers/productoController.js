...
const ProductoController = {
  createProducto: async (req, res, next) => {
    try {
      if (
        (await categoriaRepository.findById(req.body.categoria)) == undefined &&
  ...
  edit: async (req, res) => {
    try {
      let producto = await productoRepository.updateById(
        req.params.id,
        req.body
      );
      producto != undefined
  ...
  findAll: async (req, res) => {
    try {
      ...
      if (listaProductos.length == 0) {
        res.status(404).json({
          mensaje: `404: no se ha encontrado ningun elemento en la base de datos`,
        });
      } else {
  ...
  findByCategoria: async (req, res) => {
    try {
      let listaProductos = await productoRepository.findByCategoria(
        req.body.categoria
      );
      if (listaProductos.length == 0) {
        res.status(404).json({
  ...
  findById: async (req, res) => {
    try {
      let producto = await productoRepository.findById(req.params.id);
